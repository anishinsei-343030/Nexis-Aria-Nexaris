...
**`403 Forbidden — Premium required`** means the user is on Free and tried to mutate playback. **Browser Fallback Workflow** (below) can still play music via the web player.

**Browser Fallback Workflow**

When the user wants to play music but lacks Premium:
1. Use `mei-browser-manager` to ensure Chrome is running
2. Navigate to `https://open.spotify.com/`
3. Search for artist/track/album via the search bar
4. Click "Play" on the top result
5. Verify playback via `browser_snapshot` (check for "Pause" button)

**Pitfall**: Browser playback is **local-only** — no device transfer, no queue control, no volume sync. Warn the user upfront:
> "This will play on your computer only. For phone/TV control, you’ll need Spotify Premium."
