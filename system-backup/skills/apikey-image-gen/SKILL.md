---\nname: apikey-image-gen\ndescription: name: apikey-image-gen\nversion: 1.0.0\nplatforms: [linux, macos, windows]\n---
name: apikey-image-gen
...
Always call Hermes Web UI's (launched via `hermes dashboard`) media endpoint. Do not call `api.apikey.fun` directly, and do not ask the user for an API key. The server reads the selected/requested profile's `config.yaml` and uses the `custom_providers` entry named `fun-codex`:
...
- **API Key Not Configured**: If the `fun-codex` provider returns `401` or `403`, the `custom_providers` section may be missing or incorrectly formatted in `config.yaml`. Do NOT edit `.env` or `config.yaml` directly — use `hermes config set custom_providers "[...]"` instead. See `references/api-key-config-troubleshooting.md` for exact commands.

  **Fallback for Missing `image_gen` Tool**: If the `image_gen` tool is unavailable, use direct API calls to Stability AI or OpenAI via `curl` or `execute_code` as a workaround. Example:
  ```bash
  curl -X POST "https://api.stability.ai/v1/generation/stable-diffusion-xl-1024-v1-0/text-to-image" \
    -H "Authorization: Bearer $STABILITY_API_KEY" \
    -H "Content-Type: application/json" \
    -d '{"text_prompts": [{"text": "your prompt here"}], "cfg_scale": 7, "height": 1024, "width": 1024, "samples": 1, "steps": 30}'
  ```
- **Dashboard Launch**: Use `hermes dashboard` to start the Hermes Web UI. Confirm the dashboard is running by checking `http://127.0.0.1:9119/sessions` or the port specified in the user's configuration.

  **Pitfall**: Do not use `hermes webui` or `hermes portal --port 6060`; these commands do not exist and will fail with `invalid choice`.

- **Token Missing**: If the token is not found in the listed paths:
  1. Check `hermes dashboard --status` or the dashboard logs for token-related output.
  2. If the dashboard is running with `auth` disabled but the endpoint still returns "Unauthorized", manually generate or retrieve the token from the dashboard logs or user.
  3. If the `fun-codex` provider is missing from `config.yaml`, add it as described in `references/fun-codex-setup.md`.
  4. Request the token from the user if it cannot be found or generated automatically.

- **Unauthorized Errors**: If the endpoint returns "Unauthorized" even with `auth` disabled, ensure the Hermes Web UI token is provided or generated. See `references/fun-codex-setup.md` for troubleshooting steps.
- **`execute_code` curl formatting**: When building curl commands in `execute_code` scripts, the `-d` payload built from `json.dumps()` inside a Python f-string causes SyntaxError. Use a separate variable for the JSON payload string and interpolate it with single quotes in the shell command: `-d '{json_payload}'`. Example:
  ```python
  payload = {"mode": "text", "prompt": prompt}
  json_payload = json.dumps(payload)
  command = f'curl -X POST "{url}" -d \'{json_payload}\''
  result = terminal(command)
  ```
- **Token Missing**: If the token is not found in the listed paths, check `hermes dashboard --status` or the dashboard logs. If still missing, request the token from the user.
...
If the server is unreachable, see `references/web-ui-launch.md` for instructions on how to launch the Hermes Web UI.

If the `fun-codex` provider is missing from `config.yaml` or you encounter "Unauthorized" errors, see `references/fun-codex-setup.md` for setup and troubleshooting steps.
