...
### Pattern 0: Session Agent Initialization

**When to use**: First desktop tool call after Hermes restart.

```python
# First call to any desktop tool triggers Session Agent launch (~10s delay)
# Subsequent calls are instant (persistent agent)
# Always test connectivity before proceeding:

def test_session_agent():
    try:
        windows = window_list()  # Test connectivity
        if not windows:
            raise ConnectionError("Session Agent not responding")
        return True
    except Exception as e:
        raise ConnectionError(f"Session Agent unavailable: {e}")
```

**Pitfall**: WSL-based terminal commands (e.g., `hermes tools list`) **do not work** for desktop tools. Desktop tools use a **separate Session Agent** launched via `schtasks` in the user’s desktop session. Do not attempt to use WSL-dependent commands for desktop automation.
