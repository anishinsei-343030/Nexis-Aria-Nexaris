...
### 2. Broken Wikilinks & Ghost Nodes

Find `[[wikilinks]]` that point to pages that don't exist (broken links) or create "ghost nodes" in the graph (links that don't resolve to any file).

**How to check:**
- Grep for `\[\[.*?\]\]` across all pages
- Extract the link targets (strip `|...` and `#anchor`)
- For each target, check:
  - Does a file named `<target>.md` exist in any folder?
  - Does a file named `<target>/<target>.md` exist (for folder-style links)?
  - Does a file named `<folder>/<target>.md` exist (for any folder in the vault)?
- Flag any link whose target does not match any `.md` filename (stem) or `folder/filename` path in the vault

**How to fix:**
- If the target was renamed, update the link
- If the target should exist, create it
- If the link is wrong, remove or correct it
- **Ghost node prevention:** Always rename the file to match the atomic ID if you change the link target
...
Every page should have: `id`, `tags`, `created`.
...
Flag and remove wiki-specific files (`index.md`, `log.md`, `SCHEMA.md`). These files are **not needed** in long-term note-taking systems (e.g., PARA + Zettelkasten) and add maintenance overhead.

**How to check:**
- Check for existence of `index.md`, `log.md`, `SCHEMA.md` in the vault root.

**How to fix:**
- **Delete these files** to reduce maintenance overhead. If the user is transitioning from a wiki to a note-taking system, explicitly recommend removal.
- **Exception:** If `index.md` is used as a central hub (e.g., for orphaned notes), replace it with a minimal `index.md` containing only a `# Vault Index` and links to core notes (e.g., `[[3-Resources/shin]]`).

### 5. Folder Structure

Ensure flat hierarchy (max 2 subfolders deep) and PARA compliance.

**How to check:**
- List all folders and subfolders
- Flag folders deeper than 2 levels
- Check for `0-Inbox/`, `1-Projects/`, `2-Areas/`, `3-Resources/`, `4-Archive/`, `Daily/`

**How to fix:**
- Restructure folders to comply with PARA + Zettelkasten
